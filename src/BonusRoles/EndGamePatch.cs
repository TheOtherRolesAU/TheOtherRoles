...
namespace BonusRoles {
    ...
    public class OnGameEndPatch {
        public static void Postfix(AmongUsClient __instance, GameOverReason DOAMFOGADEF, bool EMAKAHIFLDE) {
            ...
            // Lovers win conditions (should be implemented using a proper GameOverReason in the future)
                ...
                if (TempData.DidHumansWin(DOAMFOGADEF)) {
            ...
            else
            {
                if (num > num2)
                {
                    ...
                    if (num + num2 == 3 && Lovers.existingAndAlive()) { // 3 players with 2 lovers is always a lover win (either shared with crewmates or solo for lovers, marked as impostor win)
                        __instance.enabled = false;
                        ShipStatus.RpcEndGame(Lovers.existingWithImpLover() ? GameOverReason.ImpostorByKill : GameOverReason.HumansByVote, !SaveManager.BoughtNoAds); // should be implemented using a proper GameOverReason in the future
                        return false;
                    }
                    return false;
                }
