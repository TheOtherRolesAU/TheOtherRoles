...
namespace BonusRoles {
    ...
    class BodyReportPatch
    {
        static void Postfix(PlayerControl __instance, GameData.PlayerInfo CAKODNGLPDF)
        {
            ...
            {
                DeadPlayer deadPlayer = deadPlayers?.Where(x => x.player?.PlayerId == CAKODNGLPDF?.PlayerId)?.FirstOrDefault();
    ...
    {
        public static void Prefix(PlayerControl __instance, PlayerControl CAKODNGLPDF, out bool __state)
        {
        ...
        public static void Postfix(PlayerControl __instance, PlayerControl CAKODNGLPDF, bool __state)
        {
            // Collect dead player info
            DeadPlayer deadPlayer = new DeadPlayer(CAKODNGLPDF, DateTime.UtcNow, DeathReason.Kill, __instance);
            ...
            if ((Lovers.lover1 != null && CAKODNGLPDF == Lovers.lover1) || (Lovers.lover2 != null && CAKODNGLPDF == Lovers.lover2)) {
                PlayerControl otherLover = CAKODNGLPDF == Lovers.lover1 ? Lovers.lover2 : Lovers.lover1;
                if (PlayerControl.LocalPlayer == CAKODNGLPDF && otherLover != null && !otherLover.Data.IsDead && Lovers.bothDie) { // Only the dead lover sends the rpc
            ...
            // Child died
            if (Child.child != null && CAKODNGLPDF == Child.child) {
