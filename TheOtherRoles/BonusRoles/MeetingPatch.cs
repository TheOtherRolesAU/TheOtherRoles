...
{
    [HarmonyPatch(typeof(MeetingHud), nameof(MeetingHud.Method_84))]
    ...
    class MeetingPopulateVotesPatch {
        static bool Prefix(MeetingHud __instance, Il2CppStructArray<byte> GPOAEGEJFCK)
        {
            // Swapper swap votes
            PlayerVoteArea swapped1 = null;
            PlayerVoteArea swapped2 = null;
            ...
            if (doSwap) {
                __instance.StartCoroutine(Effects.Slide2D(swapped1.transform, swapped1.transform.localPosition, swapped2.transform.localPosition, 2f));
                __instance.StartCoroutine(Effects.Slide2D(swapped2.transform, swapped2.transform.localPosition, swapped1.transform.localPosition, 2f));
            }
    ...
    class MeetingHudPopulateButtonsPatch {
        ...
        class MeetingHudVotingCompletedPatch {
            static void Postfix(MeetingHud __instance, byte[] GPOAEGEJFCK, GameData.PlayerInfo LNMDIKCFBAK, bool BBJAEDLJIED)
            {
                ...
                Lovers.notAckedExiledIsLover = ((Lovers.lover1 != null && Lovers.lover1.PlayerId == LNMDIKCFBAK.PlayerId) || (Lovers.lover2 != null && Lovers.lover2.PlayerId == LNMDIKCFBAK.PlayerId));
            }
        }
        ...
        static void addSwapperSwapButtons(MeetingHud __instance) {
            if (Swapper.swapper == null || PlayerControl.LocalPlayer != Swapper.swapper) return; 
        ...
        class MeetingDeserializePatch {
            static void Postfix(MeetingHud __instance, MessageReader ALMCIJKELCP, bool BILBBBFMCOB)
            {
                // Add swapper buttons
                if (BILBBBFMCOB) {
                    addSwapperSwapButtons(__instance);
    ...
    class MeetingExiledTextPatch
    {
        static void Postfix(ref string __result, StringNames HKOIECMDOKL, Il2CppReferenceArray<Il2CppSystem.Object> EBKIKEILMLF)
        {
            if (ExileController.Instance != null && ExileController.Instance.exiled != null)
            {
                ...
                if (HKOIECMDOKL == StringNames.ExileTextPP || HKOIECMDOKL == StringNames.ExileTextSP) {
